...
- Assistant generative UI — render tool results as bespoke cards (trace-summary card, tool-error table, token bar / mini-waterfall) instead of prose, à la estate-agent listing cards; add `outputSchema` to `src/features/assistant/server/tools.ts` to type them. Tools already return compact summaries; this is the render layer + per-tool opt-in
