...
const CATEGORY_TONES: Record<TraceCategory, string> = {
  chat: 'border-blue-500/30 bg-blue-500/10 text-blue-700 dark:text-blue-300',
  'sub-agent': 'border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-700 dark:text-fuchsia-300',
  scheduled: 'border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300',
  webhook: 'border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300',
  background: 'border-violet-500/30 bg-violet-500/10 text-violet-700 dark:text-violet-300',
  utility: 'border-teal-500/30 bg-teal-500/10 text-teal-700 dark:text-teal-300',
  orphan: 'border-zinc-400/30 bg-zinc-400/10 text-zinc-600 dark:text-zinc-300',
}
...
export const traceColumns: ColumnDef<TraceSummary>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex min-w-0 items-center gap-1.5">
          <span className="font-mono text-[11px] text-muted-foreground">{truncateId(s.id)}</span>
          {s.hasError && (
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-1.5">
          <Badge variant="outline" className={cn('whitespace-nowrap', CATEGORY_TONES[cat])}>
            {CATEGORY_LABELS[cat]}
          </Badge>
          {llmPurpose && (
  ...
  },
  {
    ...
    cell: ({ row }) => <div className="text-right tabular-nums text-muted-foreground">{row.original.spanCount}</div>,
  },
  {
    id: 'user',
