...
export function summarize(spans: Span[]) {
  ...
  return {
    ...
    // Depth for follow-ups ("what was slow", "where did the tokens go") and the span ids
    // get_tool_result needs — reference data, not for reciting on a clean run.
    detail: {
      ...
      // Drop the root wrapper that spans ~the whole run — "slowest = the root" is never news.
      slowest: [...spans]
        .filter((s) => dur(s) < total * 0.9)
